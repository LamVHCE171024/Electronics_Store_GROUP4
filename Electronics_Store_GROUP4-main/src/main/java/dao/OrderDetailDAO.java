    ...
    public OrderDetail getOrderDetailOfEachOrder(int orderID) {
        ...
        try {
            PreparedStatement pre = conn.prepareStatement(
                    "SELECT TOP 1 od.OrderID, od.ProductID, od.Quantity, od.Price, c.[CategoryName], p.ProductName FROM OrderDetails as od\n"
            ...
            ResultSet rs = pre.executeQuery();
            if (rs.next()) {
                od = new OrderDetail(rs.getInt(1), rs.getInt(2), rs.getInt(3), rs.getLong(4), rs.getString(5),
                        rs.getString(6));
            }
            ...
            try {
                for (CartItem item : cartItems) {
                    ...
                    BigDecimal discountFactor = BigDecimal.ONE
                            .subtract(discount.divide(BigDecimal.valueOf(100), 2, BigDecimal.ROUND_HALF_UP));
